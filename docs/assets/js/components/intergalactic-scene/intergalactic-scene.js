...
class IntergalacticScene extends HTMLElement {
	...
	connectedCallback() {
		...
		// Debug flag
		const DEBUG = window.DEBUG_INTERGALACTIC_SCENE || false;
		if (DEBUG) console.log("Intergalactic Scene component initialized");
		...
		canvas.addEventListener("touchmove", (e) => {
			if (e.touches.length === 2) {
				...
				if (prevDist)
					warp = Math.max(0.5, Math.min(15, warp + (prevDist - dist) * 0.01));
				prevDist = dist;
			}
		});
...
export default IntergalacticScene;
