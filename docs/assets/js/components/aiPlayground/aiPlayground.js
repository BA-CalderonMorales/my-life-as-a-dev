...
class AIPlayground extends HTMLElement {
	...
	initEventListeners() {
		this.shadowRoot
			.querySelectorAll(".playground-nav button")
			.forEach((button) => {
	...
	async handleRepoAnalysis() {
		...
		const insightTypes = this.shadowRoot.querySelectorAll(
			'input[name="insightType"]:checked',
		);
		const resultsDiv = this.shadowRoot.getElementById("repoResults");

		if (!repoSelect || !repoSelect.value) {
			resultsDiv.innerHTML =
				'<p class="error-message">Please select a repository.</p>';
			return;
		}
		if (insightTypes.length === 0) {
			resultsDiv.innerHTML =
				'<p class="error-message">Please select at least one insight type.</p>';
			return;
		}
		...
		try {
			...
			const response = await fetch("/api/ai/repo-analysis", {
				// This endpoint needs to be implemented on your server
				method: "POST",
	...
	initSceneCreatorListeners() {
		this.fetchTemplate(
			"/assets/templates/scene-creator.html",
			"scene-creator-content",
		).then(() => {
			const generateButton = this.shadowRoot.getElementById(
				"generateSceneButton",
			);
			if (generateButton) {
				generateButton.addEventListener("click", () =>
					this.handleSceneGeneration(),
				);
			}
	...
	async handleSceneGeneration() {
		...
		if (!descriptionInput || !descriptionInput.value) {
			resultsDiv.innerHTML =
				'<p class="error-message">Please describe the scene you want to create.</p>';
			return;
		}

		resultsDiv.innerHTML =
			'<div class="loader-small">Generating scene...</div>';
		this.isLoading = true;
		...
		try {
			...
			resultsDiv.innerHTML =
				'<p class="success-message">Scene generated successfully!</p>';
		} catch (error) {
	...
	initCodeAssistantListeners() {
		this.fetchTemplate(
			"/assets/templates/code-assistant.html",
			"code-assistant-content",
		).then(() => {
			const analyzeCodeButton =
				this.shadowRoot.getElementById("analyzeCodeButton");
			if (analyzeCodeButton) {
				analyzeCodeButton.addEventListener("click", () =>
	...
	async handleCodeAnalysis() {
		...
		if (!codeInput || !codeInput.value) {
			resultsDiv.innerHTML =
				'<p class="error-message">Please enter some code to analyze.</p>';
			return;
		}
		if (!taskSelect || !taskSelect.value) {
			resultsDiv.innerHTML =
				'<p class="error-message">Please select a task.</p>';
			return;
		}
