...
class PerformanceMonitor {
	...
	monitorFrame() {
		...
		if (now - this.lastMeasureTime >= this.measureInterval) {
			...
			const currentFrameRate = Math.round(
				(this.frameCount * 1000) / elapsedTime,
			);
			...
			this.averageFrameRate =
				this.frameRates.reduce((sum, rate) => sum + rate, 0) /
	...
	updatePerformanceMode() {
		...
		if (
			previousLowPerfMode !== this.lowPerfMode ||
			previousVeryLowPerfMode !== this.veryLowPerfMode
		) {
			if (this.veryLowPerfMode) {
				logger.warn(
					`Performance very low: ${Math.round(this.averageFrameRate)} FPS - activating very low performance mode`,
				);
				this.executeCallback("onVeryLowPerformance");
			} else if (this.lowPerfMode) {
				logger.info(
					`Performance low: ${Math.round(this.averageFrameRate)} FPS - activating low performance mode`,
				);
				this.executeCallback("onLowPerformance");
			} else {
				logger.info(
					`Performance good: ${Math.round(this.averageFrameRate)} FPS - activating standard mode`,
				);
	...
	executeCallback(callbackName) {
		if (
			...
			typeof this.callbackFunctions[callbackName] === "function"
		) {
			this.callbackFunctions[callbackName]();
		}
	}
