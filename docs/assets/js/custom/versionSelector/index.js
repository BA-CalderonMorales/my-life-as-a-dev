...
class VersionSelector {
	constructor(options = {}) {
		...
		logger.debug(
			"VersionSelector initialized with site URL: " + this.siteUrl,
			"constructor",
		);
	}
	...
	async start() {
		try {
			...
			logger.error(
				`Failed to initialize version selector: ${error.message}`,
				"start",
			);
		}
	}
	...
	_populateVersionDropdown(versions) {
		...
		if (!dropdown) {
			logger.warn(
				`Version dropdown element not found with selector: ${this.options.selectorClass}`,
				"_populateVersionDropdown",
			);
			return;
		}
		...
		logger.debug(
			"Version selector dropdown updated successfully",
			"_populateVersionDropdown",
		);
	}
}
