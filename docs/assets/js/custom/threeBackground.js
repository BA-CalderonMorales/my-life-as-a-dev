...
document.addEventListener("DOMContentLoaded", async () => {
	...
	for (let i = 0; i < particleCount; i++) {
		...
		if (type === "leaf") {
			...
			material = new THREE.MeshStandardMaterial({
				...
				emissive:
					type === "crystal"
						? new THREE.Color(ghibliColors.crystals).multiplyScalar(0.1)
						: 0x000000,
				metalness: 0.7,
				roughness: 0.2,
			});
	...
	function updateConnections() {
		...
		for (let i = 0; i < particles.length && lineIndex < linePool.length; i++) {
			for (
				let j = i + 1;
				j < particles.length && lineIndex < linePool.length;
				j++
			) {
				...
				if (
					...
					(particle1Type === "spirit" || particle2Type === "spirit")
				) {
					...
					line.material.color.setHex(
						parseInt(ghibliColors.connections.replace("#", "0x")),
		...
		particles.forEach((particle) => {
			...
			if (type === "spirit") {
				particle.material.emissive.lerp(
					targetColor.clone().multiplyScalar(0.2),
					0.1,
				);
			}
		});
	...
	function updateOpacity() {
		...
		const currentOpacity =
			parseFloat(container.style.opacity) || defaultOpacity;
	...
	function animate(currentTime) {
		...
		particles.forEach((particle, index) => {
			...
			const baseX =
				...
				Math.sin(time * userData.baseSpeed + userData.phaseX) *
					userData.amplitude;
			const baseY =
				userData.originalPosition.y +
				Math.cos(time * userData.baseSpeed + userData.phaseY) *
					userData.amplitude;
			const baseZ =
				userData.originalPosition.z +
				Math.sin(time * userData.baseSpeed + userData.phaseZ) *
					userData.amplitude *
					0.3;
			...
			particle.position.set(baseX + scrollX, baseY + scrollY, baseZ + scrollZ);
			...
			if (userData.type === "leaf") {
				...
				const glowIntensity =
					0.1 + Math.sin(time * 3 + userData.pulsePhase) * 0.1;
			...
			particle.material.opacity =
				...
				(0.8 + Math.sin(time + userData.pulsePhase) * 0.2);
		});

		// Update connections with performance throttling
		if (Math.floor(time * 30) % 10 === 0) {
			// More frequent updates for dynamic effect
			updateConnections();
		}
