...
class SectionTransitions {
	constructor() {
		...
		this.options.reducedMotion = window.matchMedia(
	...
	createObservers() {
		...
		this.observers.section = new IntersectionObserver(
			this.handleSectionIntersect,
			{
		...
		this.observers.element = new IntersectionObserver(
			this.handleElementIntersect,
			{
	...
	startWatchingElements(section) {
		...
		section.animatedElements.forEach((element, index) => {
			...
			const delay = this.options.reducedMotion
				? 0
				: this.options.animationDelay +
					index * this.options.animationDelayIncrement;
