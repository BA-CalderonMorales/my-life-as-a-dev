...
const canUseModules =
	typeof document !== "undefined" && "noModule" in HTMLScriptElement.prototype;
...
(function () {
	...
	class TypewriterCdnModel {
		constructor() {
			this.cdnUrls = ["/assets/js/vendor/typewriter.min.js"];
		}
			...
			return new Promise((resolve, reject) => {
				...
				script.onload = () => {
					...
					if (window.Typewriter) {
						...
						this.logError(
							`Typewriter.js loaded from ${url} but Typewriter object is not available`,
						);
						reject(
							new Error(
								`Typewriter object not available after script load from ${url}`,
							),
						);
					}
				};
	...
	class TypewriterLoaderController {
		...
		handleLoadError(error) {
			...
			if (logger.error) {
				logger.error(
					"All attempts to load Typewriter.js failed",
					"initialize",
					error,
				);
			} else {
