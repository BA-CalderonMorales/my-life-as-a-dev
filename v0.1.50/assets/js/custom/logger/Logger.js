...
class Logger {
  ...
  _disableConsoleLogs() {
    ...
    console.error = (msg) => {
      // Filter out common API errors that aren't critical
      if (typeof msg === 'string' && 
         ((msg.includes('api.github.com') && msg.includes('404')) || 
          (msg.includes('Failed to load resource') && msg.includes('404')) ||
          msg.includes('versions.json'))) {
        // Log these as warnings instead of errors
        this.originalConsole.warn('[Filtered Error]', msg);
        return;
      }
