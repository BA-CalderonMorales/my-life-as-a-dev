...
impl Startup {
    ...
    pub fn new(draft_version: Option<String>) -> Self {
        ...
        let (project_root, script_path) = if current_dir.ends_with("scripts/rust") || current_dir.ends_with("scripts\\rust") {
            // Running from scripts/rust directory
            (current_dir.parent().and_then(|p| p.parent()).unwrap_or_else(|| ".".as_ref()).to_path_buf(),
             current_dir)
        } else if current_dir.ends_with("scripts") {
            // Running from scripts directory
            (current_dir.parent().unwrap_or_else(|| ".".as_ref()).to_path_buf(),
    ...
    fn install_dependencies(&self) {
        ...
        println!("\nInstalling dependencies from {}...", requirements_path.display());
        
        if !requirements_path.exists() {
            eprintln!("Warning: Requirements file not found at {}", requirements_path.display());
        ...
        let status = match status {
            Ok(s) => s,
            Err(_) => {
                println!("python3 not found, trying with python...");
                Command::new("python")
                    .args(&["-m", "pip", "install", "-r"])
                    .arg(&requirements_path)
                    .status()
                    .unwrap_or_else(|_| {
                        eprintln!("Error: Failed to execute pip install command");
                        std::process::exit(1);
                    })
            }
        };
    ...
    fn check_port_and_kill_if_needed(&self) {
        ...
        let is_port_in_use = if cfg!(windows) {
            ...
            match output {
                ...
                _ => {
                    println!("Warning: Could not check if port {} is in use. Continuing...", port);
                    return;
                }
            }
        } else {
        ...
        if answer.trim().to_lowercase() == "y" {
            ...
            println!("Port {} is still in use. MkDocs server may fail to start.", port);
        }
    }
    ...
    fn kill_process_on_port(&self, port: u16) {
        ...
        if cfg!(windows) {
            ...
            let output = Command::new("cmd")
                .args(&["/C", &format!("for /f \"tokens=5\" %a in ('netstat -ano ^| findstr :{}') do @echo %a", port)])
                .output();
    ...
    fn start_documentation_server(&self) -> std::io::Result<std::process::ExitStatus> {
        ...
        if !Path::new("mkdocs.yml").exists() && !Path::new("mkdocs.yaml").exists() {
            ...
            eprintln!("Error: {}", err);
            eprintln!("Please ensure you're in the correct directory with your MkDocs documentation.");
            return Err(std::io::Error::new(std::io::ErrorKind::NotFound, err));
        }
        
        // Try different ways to run MkDocs
        let mut command = if cfg!(windows) {
            // On Windows, try to use the Python executable that has MkDocs installed
            ...
            // On Unix-like systems, try the direct mkdocs command first
