...
Rest shows zero root ink; scrolling grows tier by tier; the grown
state draws every stroke; reduced motion shows the full static system.
Mobile-standard runs first because it is the reference viewport.
"""
...
def test_rest_hides_every_root(browser, base_url):
    ...
    for selector in (".life-tree__root", ".life-roots-field__root"):
...
def test_growth_is_tier_ordered(browser, base_url):
    ...
    tree_paths = _offsets(page, ".life-tree__root")
    primary = [p for p in tree_paths if p["width"] >= 5.0]
    fine = _offsets(page, ".life-tree__root--fine")
    assert len(primary) >= 4 and fine, "expected heavy wood and feeders present"
